...
def deltanet_fused_chunked_fwd(
    ...
    initial_state: nl.ndarray,  # (128, 128) float32 — recurrent state seed (zero-tensor for fresh prefill)
):
    ...
    Input requirements:
      ...
      - initial_state seeds the SBUF recurrent state. Pass a (128, 128) zero
        tensor for fresh prefill; for hybrid-cache prefill pass the prior
        recurrent state from the cache so chunked prefill resumes correctly.
    ...
    # Seeded from `initial_state` (HBM); callers pass a zero tensor for
    # fresh prefill or the prior state for hybrid-cache prefill.
    # ================================================================
